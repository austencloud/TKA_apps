...
class KaleidoscopeTab(QWidget):
    ...
    def open_file(self):
        ...
        if filename != '':
            url = QUrl.fromLocalFile(quote(filename))  # Use urllib.parse.quote to handle special characters
    ...
    def process_and_play(self):
        ...
        self.worker = MirroringWorker(filename, temp_filename)
        self.worker.progress_update.connect(self.update_progress_bar)
        self.worker.result_ready.connect(self.finalize_mirror)
        self.worker.second_operation_started.connect(self.reset_progress_bar)
        self.worker.start()
    ...
    def update_progress_bar(self, progress):
        if self.progressBar2.isHidden():
            # If the second progress bar is hidden, update the first one
            self.progressBar.setValue(progress)
        else:
            # If the second progress bar is visible, update it instead
            self.progressBar2.setValue(progress)

    def finalize_mirror(self, processed_filename):
        # Here you can hide the progress bar, enable the play and mirror buttons, set the new media file to the player, etc.
        self.progressBar.hide() 
        url = QUrl.fromLocalFile(quote(processed_filename))  # Use urllib.parse.quote to handle special characters
        self.mediaPlayer.setMedia(QMediaContent(url))
